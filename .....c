...
// Inactivity timeouts per role (ms)
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  // Check on mount/role change if already timed out
  useEffect(() => {
    if (!user || !role) return;
  ...
  const handleLogout = async () => {
    ...
    await signOut();
    navigate(isBiker ? "/bikers-login" : isAdmin ? "/admin-login" : "/");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <SessionExpiredModal open={sessionExpired} role={role} />
